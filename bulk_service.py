...
BULK_INCLUDE_BALANCES = os.getenv("BULK_INCLUDE_BALANCES", "false").lower() == "true"
        ...
        async def worker():
            while True:
                ...
                try:
                    ...
                    balances = {}
                    if BULK_INCLUDE_BALANCES and GOLDRUSH_API_KEY and data.get("all_wallets"):
...
async def export_job_csv(user_id: int, job_id: int) -> bytes | None:
    ...
    for row in rows:
        ...
        writer.writerow([
            ...
            " | ".join(platforms),
            " | ".join(matched),
            " | ".join(wallet_balances),
            " | ".join(top_tokens[:10]),
            " | ".join(list(dict.fromkeys(chains))),
            row["elapsed_ms"] or 0,
            row["error"] or "",
        ])
