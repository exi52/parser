...
async def check_web3bio(client, username, variants):
    """Web3.bio — агрегатор профилей, возвращает адрес через API"""
    for v in variants["base"][:3]:
        try:
            ...
            if r.status_code == 200:
                ...
                wallets  = list(dict.fromkeys(
                    p.get("address") for p in profiles
                    if p.get("address") and p["address"] != "0x0000000000000000000000000000000000000000"
                ))
                if wallets:
                    info = " | ".join(
                        f"{p.get('platform','')}: {p.get('handle','')}"
                        for p in profiles if p.get("platform"))[:150]
                    return {"found": True, "platform": "Web3.bio", "emoji": "🌐",
                            "url": f"https://web3.bio/{v}", "matched": v,
                            "wallets": wallets[:5], "extra": {"профили": info}}
        except Exception:
            pass
