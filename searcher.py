...
SEARCH_CACHE_NEGATIVE_SECONDS = int(os.getenv("SEARCH_CACHE_NEGATIVE_SECONDS", "1800"))
PLATFORM_TIMEOUT_SECONDS      = int(os.getenv("PLATFORM_TIMEOUT_SECONDS", "16"))
OPENSEA_API_KEY               = os.getenv("OPENSEA_KEY", "")

_SEARCH_CACHE:    dict[str, tuple[float, dict]] = {}
_NEGATIVE_CACHE:  dict[str, float]              = {}   # username → expires_at

# ── Rate limiting ──────────────────────────────────────────────────────────────
_RATE_LIMIT_WINDOW  = 60   # секунд
_RATE_LIMIT_MAX     = 5    # запросов в окне
...
def check_rate_limit(user_id: int) -> bool:
    ...
    now  = time.time()
    reqs = _user_requests[user_id]
    # Удаляем старые
    _user_requests[user_id] = [t for t in reqs if now - t < _RATE_LIMIT_WINDOW]
    if len(_user_requests[user_id]) >= _RATE_LIMIT_MAX:
        return False
...
async def check_spaceid(client, username, variants):
    ...
    for name in target[:4]:
        try:
            r = await client.get(f"https://api.web3.bio/profile/{name}", timeout=10)
            if r.status_code != 200:
                continue
            data     = r.json()
...
async def check_web3bio(client, username, variants):
    ...
    for v in variants["base"][:3]:
        try:
            r = await client.get(f"https://api.web3.bio/profile/{v}", timeout=10)
            if r.status_code != 200:
                continue
            data     = r.json()
            profiles = data if isinstance(data, list) else [data]
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                return {"found": True, "platform": "GitHub", "emoji": "🐙",
                        ...
                        "wallets": addrs,  # может быть пустым — это нормально
...
async def _rev_web3bio(client, address):
    try:
        r = await client.get(f"https://api.web3.bio/profile/{address}", timeout=10)
        if r.status_code == 200:
...
def _is_negative_cached(username: str) -> bool:
    exp = _NEGATIVE_CACHE.get(username.lower())
    if exp and time.time() < exp:
        return True
    _NEGATIVE_CACHE.pop(username.lower(), None)
    return False


def _set_negative_cache(username: str):
    if SEARCH_CACHE_NEGATIVE_SECONDS > 0:
        _NEGATIVE_CACHE[username.lower()] = time.time() + SEARCH_CACHE_NEGATIVE_SECONDS
...
async def check_basename(client, username, variants):
    """Base Names (.base.eth) — L2 экосистема Coinbase"""
    for v in [variants["clean"]] + variants["base"][:2]:
        if not v:
            continue
        name = f"{v}.base.eth"
        try:
            # Base использует стандартный ENS resolver через Base RPC
            r = await client.post(
                "https://api.thegraph.com/subgraphs/name/ensdomains/ens",
                json={"query": f'{{domains(where:{{name:"{name}"}}){{name owner{{id}} resolvedAddress{{id}}}}}}'},
                timeout=10)
                ...
                if doms:
                    ...
                    if wallet and is_real_wallet(wallet):
                        return {
                            ...
                            "url": f"https://www.base.org/name/{v}", "matched": v,
...
PLATFORMS = [
    ...
    check_snapshot,
    check_blur_api,
    check_friendtech,     # новый — friend.tech
    check_aptos_names,    # новый — Aptos
    check_sui_names,      # новый — Sui
...
async def _run_platform(fn, client, username: str, variants: dict) -> dict:
...
async def run_search(username: str) -> dict:
    ...
    # Негативный кэш — если недавно уже ничего не нашли
    if _is_negative_cached(username):
        return {
            "username":        username,
            "variants":        [],
            "domains_checked": [],
            ...
            "found_count":     0,
            "all_wallets":     [],
            "diagnostics":     {"elapsed_ms": 0, "platforms_checked": 0, "errors": []},
            "cache_hit":       True,
            "negative_cache":  True,
        }
    ...
    if len(found) > 0:
        ...
        # Ничего не нашли — кладём в негативный кэш
        _set_negative_cache(username)
