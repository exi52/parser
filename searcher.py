...
# ─── Список платформ (только API-чекеры) ─────────────────────────────────────
...
async def run_search(username: str) -> dict:
    ...
    async with httpx.AsyncClient(headers=HEADERS, follow_redirects=True, timeout=15) as client:
        raw = await asyncio.gather(
            *[fn(client, username, variants) for fn in PLATFORMS],
            return_exceptions=True
        )

    results = [r for r in raw if r and not isinstance(r, Exception)]
    found   = [r for r in results if r.get("found")]
    wallets = list(dict.fromkeys(w for r in found for w in (r.get("wallets") or []) if w))
...
PLATFORMS = [
    check_farcaster,    # API — точный результат
    check_ens,          # API — точный результат
    check_sns,          # API — точный результат
    check_unstoppable,  # API — точный результат
    check_lens,         # API — точный результат
    check_spaceid,      # API — проверка handle
    check_web3bio,      # API — проверка handle
    check_github,       # API — адреса из bio
    check_gitcoin,      # API — точный результат
    check_snapshot,     # API — точный результат
    check_blur_api,     # API — точный результат
    # check_opensea_html  — убран: парсит чужие адреса
    # check_rainbow_html  — убран: парсит чужие адреса
    # check_zapper_html   — убран: парсит чужие адреса
]
