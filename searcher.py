...
async def fetch_wallet_balance(client: httpx.AsyncClient, address: str) -> dict:
    ...
    if not GOLDRUSH_API_KEY:
        return _blank_balance(address, "no_api_key")
    if not _is_evm(address):
        return _blank_balance(address, "non_evm")
...
async def enrich_balances(addresses: list[str]) -> dict[str, dict]:
    ...
    if not uniq:
        return out
    if not GOLDRUSH_API_KEY:
        return {a: _blank_balance(a, "no_api_key") for a in uniq}

    sem = asyncio.Semaphore(GOLDRUSH_CONCURRENCY)
    async with httpx.AsyncClient(follow_redirects=True, timeout=GOLDRUSH_TIMEOUT) as client:
