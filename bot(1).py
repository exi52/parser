...
from searcher import extract_username, run_search, run_bulk_search, reverse_lookup, is_eth_address, get_variants
...
BULK_PLANS = {
    "bulk_24h": {"name": "Bulk · 24 часа", "price": 19,  "days": 1},
    "bulk_1w":  {"name": "Bulk · 7 дней",  "price": 49,  "days": 7},
    "bulk_1m":  {"name": "Bulk · 30 дней", "price": 129, "days": 30},
}
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "bulk_info":
        ...
        if status["active"]:
            until = "навсегда" if status["forever"] else status["until_str"]
            await q.edit_message_text(
                "<b>📦 Bulk по файлу</b>\n\n"
                ...
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("➕ Продлить", callback_data="bulk_buy")],
                    [InlineKeyboardButton("← Назад",     callback_data="back_main")],
                ]))
            return
        await q.edit_message_text(
            ...
            "пачкой и возвращает CSV с кошельками и платформами.\n\n"
            f"Лимит: до {MAX_BULK_LINES} строк за заход, файл до "
            f"{BULK_MAX_FILE_BYTES // 1024 // 1024} MB.\n\n"
            "Оплачивается отдельно от обычной подписки.",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([
    ...
    if data.startswith("bulkpay:"):
        ...
        req = await request_bulk_payment(
            uid, plan_id, p.get("price", 0), p.get("days", 1), network)
...
def bulk_result_to_csv(results: list[dict]) -> bytes:
    output = io.StringIO()
...
async def run_bulk_job(ctx: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int, usernames: list[str], status_message_id: int):
    ...
    try:
        async with _bulk_slots:
            ...
            csv_bytes = bulk_result_to_csv(results)
...
async def handle_document(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    status = await update.message.reply_text(
        ...
        f"Workers: <b>{BULK_WORKERS}</b>{note}",
        parse_mode=ParseMode.HTML,
    )
...
async def cmd_confirmbulk(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if res["days"] >= 36500:
        ...
        until = res["expires"].strftime("%d.%m.%Y %H:%M")
    await update.message.reply_text(
    ...
    try:
        await ctx.bot.send_message(
            ...
            f"Доступ к загрузке файлов активен до <b>{until}</b>.\n"
            f"Пришли .txt/.csv со списком @username.",
            parse_mode=ParseMode.HTML,
            reply_markup=kb_main())
    except Exception:
        pass
...
async def cmd_grantbulk(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Выдать bulk вручную: /grantbulk USER_ID DAYS"""
    if not is_admin(update.effective_user.id):
        return
    if len(ctx.args) < 2:
        await update.message.reply_text("Формат: /grantbulk USER_ID DAYS")
        return
    try:
        user_id, days = int(ctx.args[0]), int(ctx.args[1])
    except ValueError:
        await update.message.reply_text("USER_ID и DAYS должны быть числами.")
        return
    exp = await grant_bulk_access(user_id, days)
    until = "навсегда" if days >= 36500 else exp.strftime("%d.%m.%Y %H:%M")
    await update.message.reply_text(
        f"✅ Bulk выдан <code>{user_id}</code> до <b>{until}</b>.", parse_mode=ParseMode.HTML)
    try:
        await ctx.bot.send_message(
            user_id,
            f"✅ Тебе включили <b>Bulk по файлу</b> до <b>{until}</b>.",
