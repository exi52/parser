...
def generate_ref(user_id: int) -> str:
    """Генерирует реф-код для пользователя"""
    db = load_db()
    uid = str(user_id)
    if uid not in db["users"]:
        return None
    # Если уже есть реф-код — возвращаем его
    ref = db["users"][uid].get("ref_code")
    if not ref:
        chars = string.ascii_uppercase + string.digits
        ref = "REF" + "".join(secrets.choice(chars) for _ in range(8))
        db["users"][uid]["ref_code"] = ref
        save_db(db)
    return ref


def activate_ref(user_id: int, username: str, ref_code: str) -> tuple[bool, str]:
    """Активирует доступ по реф-коду"""
    db = load_db()
    uid = str(user_id)
    ...
    for u_id, u_data in db["users"].items():
        if u_data.get("ref_code") == ref_code:
            ref_owner_uid = u_id
            break

    if not ref_owner_uid:
        return False, "Реф-код не найден."

    # Нельзя использовать свой же реф-код
    if ref_owner_uid == uid:
        return False, "Нельзя использовать собственный реф-код."

    # Проверяем что владелец активен
    owner = db["users"][ref_owner_uid]
    ...
    db["users"][uid] = {
        ...
        "active": True,
        "searches": 0,
        "invited_by": ref_owner_uid,
        "invited_by_name": owner.get("username", ""),
    }

    # Записываем в список рефералов владельца
    if "referrals" not in db["users"][ref_owner_uid]:
        db["users"][ref_owner_uid]["referrals"] = []
    db["users"][ref_owner_uid]["referrals"].append({
        "user_id": user_id,
        "username": username,
    ...
    return True, f"Доступ активирован! Тебя пригласил @{owner_name}."
...
def get_referrals(user_id: int) -> list:
    ...
    if uid not in db["users"]:
        return []
    return db["users"][uid].get("referrals", [])


def list_all_refs() -> list:
    """Для админа — все цепочки рефералов"""
    db = load_db()
    result = []
    for uid, user in db["users"].items():
