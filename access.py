...
# ─── Bulk: отдельная платная функция (поиск по файлу) ─────────────────────────
# Доступ хранится в users.bulk_expires, оплата идёт через payments (product='bulk').
# Подписочные функции выше отфильтрованы по product='sub', поэтому потоки не пересекаются.

BULK_FOREVER_DAYS = 36500


def _bulk_expiry(current, days: int):
    now = datetime.now()
    if days >= BULK_FOREVER_DAYS:
        return now + timedelta(days=BULK_FOREVER_DAYS)
...
async def check_bulk_access(user_id: int) -> bool:
    pool = await get_pool()
    async with pool.acquire() as conn:
        exp = await conn.fetchval("SELECT bulk_expires FROM users WHERE user_id=$1", user_id)
...
async def get_bulk_status(user_id: int) -> dict:
    ...
    if not exp or datetime.now() >= exp:
        return {"active": False, "until": None, "until_str": None, "forever": False}
    forever = (exp - datetime.now()).days > 30000
    return {"active": True, "until": exp, "until_str": _fmt_exp(exp), "forever": forever}


async def grant_bulk_access(user_id: int, days: int) -> datetime:
    """Выдать/продлить bulk. Создаёт юзера, если его ещё нет. Возвращает expires (datetime)."""
    pool = await get_pool()
    async with pool.acquire() as conn:
        async with conn.transaction():
            cur = await conn.fetchval("SELECT bulk_expires FROM users WHERE user_id=$1", user_id)
            expires = _bulk_expiry(cur, days)
            await conn.execute("""
                INSERT INTO users (user_id, ref_code, bulk_expires)
                VALUES ($1, $2, $3)
                ON CONFLICT (user_id) DO UPDATE SET bulk_expires=$3
            """, user_id, _gen_code("REF", 8), expires)
    return expires
...
async def revoke_bulk_access(user_id: int) -> bool:
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            cur = await conn.fetchval("SELECT bulk_expires FROM users WHERE user_id=$1", user_id)
            had = bool(cur and cur > datetime.now())
            await conn.execute("UPDATE users SET bulk_expires=NULL WHERE user_id=$1", user_id)
            await conn.execute("""
...
async def request_bulk_payment(user_id: int, plan_id: str, amount: float, days: int, network: str) -> dict:
    pool = await get_pool()
    address = WALLETS.get(network, WALLETS["eth"])
    async with pool.acquire() as conn:
        async with conn.transaction():
...
async def confirm_bulk_payment(user_id: int) -> dict | None:
    """Подтвердить bulk-оплату вручную (админ). Продлевает users.bulk_expires."""
    pool = await get_pool()
    async with pool.acquire() as conn:
        async with conn.transaction():
            payment = await conn.fetchrow("""
            ...
            days = payment["subscription_days"] or 7
            cur = await conn.fetchval("SELECT bulk_expires FROM users WHERE user_id=$1", user_id)
            expires = _bulk_expiry(cur, days)
            await conn.execute("UPDATE users SET bulk_expires=$1 WHERE user_id=$2", expires, user_id)
            await conn.execute("""
                UPDATE payments
                SET confirmed=TRUE, status='confirmed', updated=NOW()
                WHERE id=$1
