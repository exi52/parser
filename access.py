...
WALLETS = {
    ...
    "usdc_sol":   os.getenv("WALLET_USDC_SOL", os.getenv("WALLET_SOL", "ВСТАВЬ_USDC_SOL")),
...
NETWORK_INFO = {
    ...
    "usdc_sol":   {"name": "USDC SOL",    "symbol": "USDC", "network": "Solana",    "emoji": "🔵"},
    "sol":        {"name": "SOL",         "symbol": "SOL",  "network": "Solana",    "emoji": "🟣"},
    "ton":        {"name": "TON",         "symbol": "TON",  "network": "TON",       "emoji": "💎"},
}

PAYMENT_ADDRESS = WALLETS["usdt_trc20"]
...
async def grant_trial_searches(user_id: int, count: int, hours: int = TRIAL_HOURS) -> dict:
    """Manually grant trial searches without activating a paid subscription."""
    if count <= 0:
        raise ValueError("count must be positive")
    if hours <= 0:
        raise ValueError("hours must be positive")

    trial_exp = datetime.now() + timedelta(hours=hours)
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute("""
            INSERT INTO users (user_id, username, active, trial_searches, trial_expires, ref_code, blocked)
            VALUES ($1, $2, FALSE, $3, $4, $5, FALSE)
            ON CONFLICT (user_id) DO UPDATE
            SET trial_searches=$3,
                trial_expires=$4,
                username=COALESCE(users.username, $2)
        """, user_id, str(user_id), count, trial_exp, _gen_code("REF", 8))

    return {"user_id": user_id, "count": count, "hours": hours, "expires": trial_exp}
...
async def get_user_stats(user_id: int) -> dict:
    ...
    sub_active = bool(user["active"] and (not user["subscription_expires"] or datetime.now() < user["subscription_expires"]))
    return {
        "active":       user["active"],
        "blocked":      user["blocked"],
        "has_key":      bool(user["key"]),
        "sub_active":   sub_active,
        "sub_exp":      _fmt_exp(user["subscription_expires"]) if sub_active else None,
...
async def create_payment(user_id: int, plan_id: str, amount: float, days: int, network: str) -> dict:
    pool = await get_pool()
    address = WALLETS.get(network, WALLETS["usdt_trc20"])
    async with pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute("""
                UPDATE payments
...
async def grant_subscription(user_id: int, days: int, plan_id: str | None = None) -> dict:
    """Manually grant regular parser access for a chosen number of days."""
    if days <= 0:
...
async def get_admin_stats() -> dict:
    """Return high-level admin stats for users, payments and revenue."""
    pool = await get_pool()
    async with pool.acquire() as conn:
        users = await conn.fetchrow("""
            SELECT
                COUNT(*)::BIGINT AS users_total,
                COUNT(*) FILTER (WHERE blocked = TRUE)::BIGINT AS users_blocked,
                COUNT(*) FILTER (
                    WHERE active = TRUE
                      AND (subscription_expires IS NULL OR subscription_expires > NOW())
                )::BIGINT AS active_subscriptions,
                COUNT(*) FILTER (
                    WHERE trial_searches > 0 AND trial_expires > NOW()
                )::BIGINT AS active_trials,
                COUNT(*) FILTER (
                    WHERE COALESCE(bulk_credits, 0) > 0
                )::BIGINT AS bulk_active_users
            FROM users
        """)
        payments = await conn.fetchrow("""
            WITH confirmed_payments AS (
                SELECT
                    ...
                    COALESCE(NULLIF(product, ''), 'sub') AS product,
                    COALESCE(updated, created) AS paid_at
                FROM payments
                WHERE (status = 'confirmed' OR confirmed = TRUE)
            ...
            SELECT
                ...
                )::BIGINT AS payments_month,
                COUNT(DISTINCT user_id)::BIGINT AS paying_users_total,
                COUNT(DISTINCT user_id) FILTER (
                    WHERE paid_at >= date_trunc('month', NOW())
                )::BIGINT AS paying_users_month,
                ...
                COALESCE(SUM(amount) FILTER (
                    WHERE product = 'sub'
                ), 0) AS sub_revenue_total,
                COALESCE(SUM(amount) FILTER (
                    WHERE product = 'sub'
                      AND paid_at >= date_trunc('month', NOW())
                ), 0) AS sub_revenue_month,
                COALESCE(SUM(amount) FILTER (
                    WHERE product = 'bulk'
                ), 0) AS bulk_revenue_total,
                COALESCE(SUM(amount) FILTER (
                    WHERE product = 'bulk'
                      AND paid_at >= date_trunc('month', NOW())
                ), 0) AS bulk_revenue_month
            FROM confirmed_payments
        """)

    stats = {}
    for row in (users, payments):
        for key, value in dict(row or {}).items():
            stats[key] = float(value) if hasattr(value, "as_tuple") else int(value or 0)
    return stats
