...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.args:
        ...
        if arg.startswith("REF"):
            ok, msg = activate_ref(uid, uname, arg)
        ...
        # Ключ доступа
        ok, msg = activate_key(uid, uname, arg)
        await update.message.reply_text(
            f"{'✅' if ok else '❌'} {esc(msg)}", parse_mode=ParseMode.HTML)
        if ok and uid != ADMIN_ID:
            try:
...
async def cmd_myref(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    ok, _ = check_access(uid)
    if not ok and uid != ADMIN_ID:
    ...
    lines = [
        ...
        "",
        f"Приглашено: <b>{len(referrals)}</b> чел.",
    ]
    if referrals:
        lines.append("")
...
async def cmd_refs(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
