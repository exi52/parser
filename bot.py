...
# Хранит выбранный тариф пока пользователь не оплатил (в памяти, сессионно)
user_plan_choice: dict[int, str] = {}
# Хранит выбранную сеть
user_network_choice: dict[int, str] = {}
# Ждём хеш транзакции от этих пользователей
awaiting_hash: set[int] = set()
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        stats = await get_user_stats(uid)
        if has_access:
            status = "✅ Подписка" if stats.get("has_key") else f"⏳ Пробный: {stats.get('trial_left',0)} поисков"
        else:
            status = "❌ Нет подписки"
        await q.edit_message_text(
            f"<b>{'👑 Панель администратора' if is_admin else '🔍 Crypto OSINT Bot'}</b>"
            + (f"\n\nСтатус: {status}" if not is_admin else ""),
            parse_mode=ParseMode.HTML,
    ...
    if data == "stats":
        stats = await get_user_stats(uid)
        text  = "<b>📊 Статистика</b>\n\n"
        text += f"Подписка: {'✅ Активна' if stats.get('has_key') else '❌ Нет'}\n"
    ...
    if data.startswith("pay_"):
        ...
        user_plan_choice[uid]    = plan_id
        user_network_choice[uid] = network
        ...
        await create_payment(uid, network)
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in awaiting_hash:
        awaiting_hash.discard(uid)
        plan_id  = user_plan_choice.get(uid, "?")
        network  = user_network_choice.get(uid, "?")
